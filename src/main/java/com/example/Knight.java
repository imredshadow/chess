...
public class Knight extends Piece {
    ...
    @Override
    public ArrayList<Square> getControlledSquares(Square[][] board,
            Square start) {
        int x = start.getCol();
        int y = start.getRow();
        // List of controlled squares that shall be returned
        ArrayList<Square> controlledSquares = new ArrayList<Square>();
        int[] signs = { 1, -1 }; // This is used later in for each loops to make itso the knight moves in
        ...
        // and 1 to the side or 2 to theside and 1 up. This is used in the for
        // each loop to make my code moreunnecessarily compact
        // Copious amounts of for each loops to loop through all possible moves ofthe
        // knight
        for (int sign1 : signs) {
            for (int sign2 : signs) {
                for (int[] pair : pairs) {
